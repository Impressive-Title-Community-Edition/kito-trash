...
public:
	...
	void updateCritterOwnership(const FrameEvent &evt)
	{
		...
		for(int i=0;i<(int)tAbandonList.size();i++)
		{
			...
			if(tSuiter)mNetworkManager->sendCritterTakeover(tC->getID(),(OwnerToken)tSuiter->getIndex());
		}
	}
	void updateCritters(const FrameEvent &evt)
	{
		...
		if(spawnTimer<=0 && mCritterManager->getNumCritters()<mWorld->getCritterSpawnLimit())
		{
			...
			if(tList.size()>0)
			{
				unsigned short tID = (unsigned short)Math::RangeRandom(0,(Ogre::Real)tList.size());
				...
				if(Math::UnitRandom()<(tCritter.spawnRate))
				{
					...
					if(mGameStateManager->isCampaign())mUnitManager->createCritter(mCritterManager->getNextEmptyID(0),(unsigned char)tID,Vector3(tX,tY,tZ));
					else mNetworkManager->sendCritterCreate((unsigned char)tID,Vector3(tX,tY,tZ));
				}
			}
		}
		...
		for(int i=0;i<(int)tCritterDecisionList.size();i++)
		{
			...
			if(tC->getIsPet() && mUnitManager->getPlayer()->getPet()==tC->getCritterType())
			{
				...
				if(!tPetFlags->stay)
				{
					...
					else
					{
						...
						Real tDist = (Ogre::Real)(tC->getLength()*0.8);
			...
			else
			{
				...
				tC->setDecisionTimer(Math::RangeRandom((Ogre::Real)tInfo.decisionMin,(Ogre::Real)(tInfo.decisionMin+tInfo.decisionDeviation)));
			}
		}
