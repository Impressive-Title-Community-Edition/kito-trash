...
public:
    ...
    bool mouseMoved(const OIS::MouseEvent &e)
	{
		...
		if(!mMagixHandler->getGameState()->isCinematic() && mMagixHandler->getGameState()->isInGame() && mMagixHandler->getCameraMode()==CAMERA_LOCKED)
		{
			...
			if( ms.buttonDown( MB_Right ) && !mMagixHandler->getCamera()->getIsFirstPerson())
			{
				...
				{
					mMagixHandler->getCamera()->doCameraZoom((Ogre::Real)((Math::Abs((Ogre::Real)ms.X.rel)>8000?0:-ms.Y.rel) * 0.0013 * mouseSense));
				}
				else
				{
					mMagixHandler->getCamera()->doCameraZoom((Ogre::Real)(-ms.Y.rel * 0.13 * mouseSense));
				}
			}
			//Rotate camera
			else if(!mMagixHandler->getPlayer()->getIsLockedOn())
			{
				if(mMagixHandler->getMagixGUI()->isTablet())
				{
					mMagixHandler->getCamera()->yaw(Degree((Ogre::Real)((Math::Abs((Ogre::Real)ms.X.rel)>8000?0:-ms.X.rel) * 0.0013 * mouseSense)));
					mMagixHandler->getCamera()->pitch(Degree((Ogre::Real)((Math::Abs((Ogre::Real)ms.Y.rel)>8000?0:-ms.Y.rel) * 0.0013 * mouseSense)));
				}
				else
				{
					mMagixHandler->getCamera()->yaw(Degree((Ogre::Real)(-ms.X.rel * 0.13 * mouseSense)));
					mMagixHandler->getCamera()->pitch(Degree((Ogre::Real)(-ms.Y.rel * 0.13 * mouseSense)));
				}
			}
		}
		else//Mouse cursor mode
		{
			...
			//zoom camera
			if(tDoZoom && !mMagixHandler->getGameState()->isCinematic() && (mMagixHandler->getGameState()->isInGame() || mMagixHandler->getGameState()->isInEditor()))
			{
				if(mMagixHandler->getMagixGUI()->getUseWindowsCursor())
				{
					unsigned int tW,tH,tCD;
			...
			//rotate view
			else if(tDoRotate && !mMagixHandler->getGameState()->isCinematic() && (mMagixHandler->getGameState()->isInGame() || mMagixHandler->getGameState()->isInEditor()))
			{
				if(mMagixHandler->getMagixGUI()->getUseWindowsCursor())
				{
					unsigned int tW,tH,tCD;
					int tL,tT;
					mWindow->getMetrics(tW,tH,tCD,tL,tT);
					SetCursorPos((int)(tL+tW*mMagixHandler->getMagixGUI()->getMouseX()),(int)(tT+tH*mMagixHandler->getMagixGUI()->getMouseY()));
					//if(ShowCursor(false)<-1)ShowCursor(true); 
				}
				mMagixHandler->getMagixGUI()->showMouse(false);
				if(mMagixHandler->getMagixGUI()->isTablet())
		...
		if(mMagixHandler->getMagixGUI()->isTablet() && !mMagixHandler->getMagixGUI()->getUseWindowsCursor())
		{
			...
			mMagixHandler->getMagixGUI()->scrollMouse( (int)(ms.X.rel * 0.015 * tXYRatio * mouseSense), (int)(ms.Y.rel * 0.015 * mouseSense), (int)mWindow->getWidth(), (int)mWindow->getHeight());
		}
    ...
    bool keyPressed(const OIS::KeyEvent &e)
	{
		...
		switch (e.key)
		{
			...
			case KC_TAB:	if(!mKeyboard->isModifierDown(Keyboard::Alt))mMagixHandler->getMagixGUI()->toggleShowGUI();	break;
		...
		//InEditor controls
		else if( !mMagixHandler->getGameState()->isCinematic() && mMagixHandler->getGameState()->isInEditor() && mMagixHandler->getInputMode()==INPUT_CONTROL)
		switch (e.key)
		{
			//Player controls
			case KC_W:
			...
			case KC_SUBTRACT:
				//mMagixHandler->getCamera()->changeSpeed(0.5);
				mMagixHandler->getWorldEditor()->setCurrentModifier(true, false);
				break;
			case KC_EQUALS:
			case KC_ADD:
				//mMagixHandler->getCamera()->changeSpeed(2);
				mMagixHandler->getWorldEditor()->setCurrentModifier(true, true);
				break;
			case KC_ESCAPE:
				mMagixHandler->getMagixGUI()->toggleShowOptions();
				if(mMagixHandler->getCameraMode()==CAMERA_LOCKED)
    ...
    bool keyReleased(const OIS::KeyEvent &e)
	{
		...
		//Ineditor controls
		if(mMagixHandler->getGameState()->isInEditor() && !mMagixHandler->getGameState()->isCinematic())
		switch (e.key)
		{
			//Player controls
			case KC_W:
			...
			case KC_MINUS:
			case KC_SUBTRACT:
				mMagixHandler->getWorldEditor()->setCurrentModifier(false, false);
				break;
			case KC_EQUALS:
			case KC_ADD:
				mMagixHandler->getWorldEditor()->setCurrentModifier(false, true);
				break;
...
//kito		mSceneMgr->setFog( FOG_LINEAR, fadeColour, .0001, 500, 1500);
		...
		if(mMagixHandler->getExternalDefinitions()->hasVertexProgram && mMagixHandler->getExternalDefinitions()->hasFragmentProgram)
		{
			...
			//Ogre::CompositorManager::getSingleton().setCompositorEnabled(pViewport, "VolumetricSunLights", false);

			//pEffectListener = new VolumetricSunLightFrameListener(mSceneMgr, mCamera, pViewPort);
			//pEffectInstance->addListener(pEffectListener);
			/*CompositorManager::getSingleton().setCompositorEnabled(mCamera->getViewport(), "Default", true);
			MaterialManager::getSingleton().setActiveScheme(Ogre::MaterialManager::DEFAULT_SCHEME_NAME);
			mCamera->getViewport()->setMaterialScheme("Default");
			mWindow->getViewport(0)->setMaterialScheme("Default");*/
		}
	}
