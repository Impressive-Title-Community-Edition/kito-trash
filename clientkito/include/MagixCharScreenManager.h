...
public:
		...
		if(button==mDeleteButton)
		{
			...
			if(page==PAGE_EDIT)
			{
				if(deleteConfirmation=="")
				{
					...
					mName->show();
					mAlertBox->showAlert("Enter character name to confirm delete",mName->getLeft()+(Ogre::Real)0.25,mName->getTop()+mName->getHeight()+(Ogre::Real)0.05);
					return 0;
				}
				if(deleteConfirmation!=getName())
				{
					mAlertBox->showAlert("Character name mismatch",mName->getLeft()+(Ogre::Real)0.25,mName->getTop()+mName->getHeight()+(Ogre::Real)0.05);
					return 0;
				}
	...
	void refreshPage()
	{
		...
		if(page==PAGE_SELECT)
		{
			...
			if(getName()!="")
			{
				...
				mDeleteButton->setPosition((Ogre::Real)0.85-mDeleteButton->getWidth()+mRButton[0]->getWidth(),0.85);
			...
			else
			{
				if(charID==numChars)
				{
					...
					mNextButton->setPosition((Ogre::Real)0.85-mNextButton->getWidth()+mRButton[0]->getWidth(),0.85);
				}
	...
	const unsigned short nextPage()
	{
		...
		if((page==PAGE_CUSTOM1 || page==PAGE_PRESET1) && (!validateName() || getName().length()==0 || getName()[0]==' '))
		{
			...
			mAlertBox->showAlert(tAlert,mName->getLeft()+(Ogre::Real)0.25,mName->getTop()+mName->getHeight()+(Ogre::Real)0.05);
			return 0;
		}
	...
	void updateSliders(const Real &one, const Real &two, const Real &three)
	{
		...
		{
			const Real tScale = (Ogre::Real)0.25 + (page==PAGE_PRESET2?two:one)*(Ogre::Real)1.25;
	...
	void getSliderValue(Real &one, Real &two, Real &three)
	{
		...
		{
			one = (mUnit->getObjectNode()->getScale().x-(Ogre::Real)0.25)/(Ogre::Real)1.25;
		}
		else if(page==PAGE_PRESET2)
		{
			two = (mUnit->getObjectNode()->getScale().x-(Ogre::Real)0.25)/(Ogre::Real)1.25;
		}
	}
	...
	bool update(const FrameEvent &evt, const Real &x, const Real &y)
	{
		...
		if(page==PAGE_SELECT)
		{
			...
			if(startGameCount>0)
			{
				...
				mBackdrop->setDimensions(1+(3-startGameCount)/3*(Ogre::Real)0.4,1+(3-startGameCount)/3*(Ogre::Real)0.6);
		...
		if(isRotateUnit)
		{
			...
			rotateStartX = x;
			rotateStartY = y;
			if(Math::Abs(dYaw)>Degree(15))dYaw = Degree(15*dYaw<Degree(0)?(Ogre::Degree)-1:(Ogre::Degree)1);
	...
	void positionUnitForSelect()
	{
		...
		if(!mUnit->getObjectNode())return;
		const Real tDist = (mUnit->getObjectNode()->getScale().x-1)*-11;
		mUnit->setPosition(3215 - (tDist<0?tDist:0)*(Ogre::Real)0.6, 113 + (tDist<0?tDist:0), 2750 + (tDist<0?tDist:0)*(Ogre::Real)0.6);
		mUnit->resetTarget();
		mUnit->setGroundHeight(113 + (tDist<0?tDist:0));
		mUnit->yaw(Degree(-80));
		mUnit->getObjectNode()->yaw(Degree(-80));
