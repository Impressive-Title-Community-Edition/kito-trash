...
public:
	...
	void updateChase()
	{
		...
		if(!hasAttack)return;
		const Real tAttackRange = length<=55.55 ? 2500 : (length*length*(Ogre::Real)0.81);
	...
	void kill()
	{
		hp = 0;
		isDead = true;
		decisionTimer = isDrawPoint?(Ogre::Real)1:(Ogre::Real)10;
	...
	void setTarget(const Vector3 &position, bool clearChaseTarget=true)
	{
		...
		if(targetPosition.x!=mObjectNode->getPosition().x || targetPosition.z!=mObjectNode->getPosition().z)
		{
			...
			Radian tYaw = Degree(0);
			if(tZ!=0)tYaw = Math::ATan(tX/tZ);
			else tYaw = Degree(tX>=0?(Ogre::Real)90:(Ogre::Real)270);
			if(tX<0)tYaw += Degree(360);
			if(tZ<0)tYaw += Degree(180);
			setYaw(Degree(tYaw));
		}
	...
	const Vector3 getPosition(bool headPosition=false)
	{
		...
		return (mObjectNode->getPosition() + (headPosition?Vector3(0,height*(Ogre::Real)0.5,0):Vector3::ZERO));
	}
...
public:
			...
			if(!tCritter->getIsDead() && !tCritter->getIsDrawPoint())
			{
				...
				if(tCritter->popIsReadyToAttack())
				{
					if(tIsInRange)
					{
						...
						const Vector3 tOffset = Vector3(0,tCritter->getHeight()*(Ogre::Real)0.25,tCritter->getLength()*(Ogre::Real)0.5);
	...
	void hitCritter(MagixCritter *critter, const Real &damage, const Vector3 &force, const OwnerToken &token=0, bool inRange=false)
	{
		...
		if(damage<0)
		{
			if(!critter->isInvulnerable())
			{
				...
				if(critter->imTheOwner() && Math::UnitRandom()<0.5)critter->setDecisionTimer(critter->getDecisionTimer()*(Ogre::Real)0.5);
			}
		}
		else if(damage>0)
		{
	...
	void updateGroundHeight(MagixCritter *critter, bool checkWaterCollision=false, bool checkCollBoxCollision=false)
	{
		...
		tHeight = mWorld->getTerrainHeight(critter->getPosition().x,critter->getPosition().z);
