...
//		if(mRayQuery)delete mRayQuery;
	...
	void initialize(SceneManager *sceneMgr, MagixPlayer *player, MagixCollisionManager *collMgr, MagixGameStateManager *GameStateManager)
	{
...
//		mRayQuery = sceneMgr->createRayQuery( Ray() );
		...
		headLight->setDiffuseColour(1.0, 1.0, 1.0);
		headLight->setSpecularColour(1.0, 1.0, 1.0);
		headLight->setAttenuation(100, 1.0, 0.045, 0.0075);
		headLight->setVisible(true);
	}
	void setTerrainGroup(TerrainGroup *TrGroup)
	{
...
//		mWorld = world;
	...
	void reset(bool resetCamZoom=true)
	{
		...
		isQuakeFadeOut = false;
		quakeOffset = Vector3::ZERO;
		if(!headLight->isAttached())
			mPlayer->getObjectNode()->attachObject(headLight);
	}
	void update(const FrameEvent &evt)
	{
		...
		{
			//Camera follows unit
			mCamPivotNode->setPosition(mPlayer->getPosition(isFirstPerson)+(isFirstPerson?mPlayer->getObjectNode()->getOrientation()*Vector3(0,0,(Ogre::Real)3.5*mPlayer->getObjectNode()->getScale().x):Vector3::ZERO));
		...
		if(turnUp)pitch(Degree(45*camSpeed*(Ogre::Real)0.02*evt.timeSinceLastFrame));
		else if(turnDown)pitch(Degree(-45*camSpeed*(Ogre::Real)0.02*evt.timeSinceLastFrame));
		if(turnLeft)yaw(Degree(45*camSpeed*(Ogre::Real)0.02*evt.timeSinceLastFrame));
		else if(turnRight)yaw(Degree(-45*camSpeed*(Ogre::Real)0.02*evt.timeSinceLastFrame));
	...
	{
		if((mGameStateManager->isInEditor() || mGameStateManager->getGameState()==GAMESTATE_STARTMAPEDITOR))
		{
			camMode = mode;
			return;
		}
	...
	void pitch(const Degree &angle)
	{
		...
		camPitch += angle;
		const Real tMaxUpPitch = (isFirstPerson?(Ogre::Real)75:(camMode==CAMERA_FREE?(Ogre::Real)90:(Ogre::Real)89));
		const Real tMaxDownPitch = (isFirstPerson?(Ogre::Real)-75:(camMode==CAMERA_FREE?(Ogre::Real)-90:(Ogre::Real)-89));
	...
	const Real getGroundHeight(const Real &x, const Real &z)
	{
		...
		return tHeight;*/
//		return mWorld->getTerrainHeight(x, z);
		if(mTerrainGroup)
		{ 
			//mTerrainGroup->getHeightAtWorldPosition(x,0,z);
			Ray ray;
	...
	{
		isQuake = flag;
		quakeCountdown = fade?(Ogre::Real)1:(Ogre::Real)0;
