...
public:
	...
	void saveSettings(bool isTablet, const Real &mouseSensitivity, const Real &musicVol, const Real &guiVol, const Real &soundVol, const unsigned short &isShadowsOn, bool isBloom, const String &username="", bool generalCharname=false, bool localUsername=false, bool channelBlink=true, bool runMode=true, bool doubleJumpOn=true, bool autoLipSync=false, bool useWindowsCursor=false)
	{
	...
	void loadCritters(const String &filename, bool isCustom)
	{
        ...
        while (seci.hasMoreElements())
        {
            ...
            for (i = settings->begin(); i != settings->end(); ++i)
            {
				...
				else if(StringUtil::startsWith(i->first,"decisionmin"))tC.decisionMin = (unsigned char)StringConverter::parseReal(i->second);
	...
	const unsigned char getCritterRandomAttack(const String &type, bool *hitAlly=0)
	{
		for(int i=0;i<(int)critterList.size();i++)
		{
			if(critterList[i].type==type)
			{
				...
				unsigned char tID = (unsigned char)Math::RangeRandom(1,(Ogre::Real)critterList[i].attackList.size());
	...
	const unsigned char getCritterRandomSpecificAttack(const String &type, bool getNonHeal)
	{
		for(int i=0;i<(int)critterList.size();i++)
		{
			if(critterList[i].type==type)
			{
				...
				unsigned char tID = (unsigned char)Math::RangeRandom(1,(Ogre::Real)tList.size());
