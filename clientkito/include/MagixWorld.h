...
protected:
	// Bullet
	OgreBulletDynamics::DynamicsWorld *mDynWorld;
	bool mDynPaused;
	std::deque<OgreBulletDynamics::RigidBody *>         mRigids;
	...
	//// Kito Terrain
	// Terrain image file name
	String mHeightmapImage;
	// Terrain max height
	int mMaxHeight;
	// Terrain max pixel error
	int mMaxPixelError;
	// Terrain size
	int mPageSize;
	// Min batch size
	...
	// Max batch size
	int mMaxBatchSize;
	// Composite map distance
	int mCompositeMapDistance;
public:
	MagixWorld()
	{
		mDef = 0;
	...
	void initialize(SceneManager *sceneMgr, RenderWindow *window, MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr, MagixSoundManager *soundMgr, MagixCollisionManager *collisionMgr, MagixSkyManager *skyMgr, TerrainGlobalOptions *TerrainGlobals, MagixCamera *camera)
	{
	...
	void update(const FrameEvent &evt)
	{
		...
		// update physics
		if (!mDynPaused)
			mDynWorld->stepSimulation(evt.timeSinceLastFrame);
	}
	...
	{
		String resgroup = ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME;
		img.load(mHeightmapImage, resgroup);
		if (flipX)
			img.flipAroundY();
		if (flipY)
			img.flipAroundX();
	...
	{
		//terrain->addLayer((uint8)1);
		//terrain->addLayer((uint8)2);
		...
		//blendMap0->loadImage("grassdensitymap.png", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
		//blendMap1->loadImage("grassdensitymap2.png", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
		blendMap0->dirty();
		blendMap1->dirty();
	...
	inline float getTerrainHeight(const float x, const float z, void *userData = NULL) /* HeightFunction.h */
	{
//		return 0;
		return mTerrainGroup->getHeightAtWorldPosition(x,0,z);
		Ray ray;
	...
	void loadConfig(const String terrain, const String name)
	{
        /* Set up the options */
        ConfigFile config;
        String val;
		
		// Create data stream pointer to the terrain cfg loaded from
		// tTerrain (worldName.cfg) in the 'name' world variable (worldName)
		DataStreamPtr stream = 
				ResourceGroupManager::getSingleton().openResource(terrain, name);
		config.load(stream);

		val = config.getSetting( "Heightmap.image" );
        if ( !val.empty() )
            setHeightmapImage(val);
		
        val = config.getSetting( "MaxHeight" );
        if ( !val.empty() )
            setMaxHeight(atoi( val.c_str() ));
		...
		val = config.getSetting( "MinBatchSize" );
        if ( !val.empty() )
            setMinBatchSize(atoi( val.c_str() ));

		val = config.getSetting( "MaxBatchSize" );
        if ( !val.empty() )
            setMaxBatchSize(atoi( val.c_str() ));

		val = config.getSetting( "CompositeMapDistance" );
        if ( !val.empty() )
            setCompositeMapDistance(atoi( val.c_str() ));
	}
	void setHeightmapImage(String heightmapImage) {
		mHeightmapImage = heightmapImage;
	}
	void setMaxHeight(int height) {
		mMaxHeight = height;
	}
	void setMaxPixelError(int pixelError) {
		mMaxPixelError = pixelError;
	}
	void setPageSize(int pageSize) {
		mPageSize = pageSize;
	}
	void setMinBatchSize(int batchSize) {
		mMinBatchSize = batchSize;
	}
	void setMaxBatchSize(int batchSize) {
		mMaxBatchSize = batchSize;
	}
	void setCompositeMapDistance(int distance) {
		mCompositeMapDistance = distance;
	}
	void initBullet()
	{
		Ogre::Vector3 &gravityVector = Ogre::Vector3 (0,-9.81,0);
		Ogre::AxisAlignedBox &bounds = Ogre::AxisAlignedBox (Ogre::Vector3 (-worldSize.x, 0, -worldSize.y),
                                                                               Ogre::Vector3 (worldSize.x,  10000,  worldSize.y));
		// Start Bullet
		mDynWorld = new OgreBulletDynamics::DynamicsWorld (mSceneMgr, bounds, gravityVector, true, true, 10000);
	}
	void loadWorld(const String &name)
	{
		...
		// load terrain config that was previously done by the OctreeSceneManager
		loadConfig(tTerrain, name);
		...
		if(worldSize.x <= worldSize.y) {
			mTerrainGroup = OGRE_NEW Ogre::TerrainGroup(mSceneMgr, Terrain::ALIGN_X_Z, 513, worldSize.y);
			mTerrainGroup->setOrigin(Vector3(worldSize.y/2,0,worldSize.y/2)); // This is the CENTER
		} else /*if(worldSize.x > worldSize.y)*/ {
			mTerrainGroup = OGRE_NEW Ogre::TerrainGroup(mSceneMgr, Terrain::ALIGN_X_Z, 513, worldSize.x);
			mTerrainGroup->setOrigin(Vector3(worldSize.x/2,0,worldSize.x/2)); // This is the CENTER
		} /*else {
			mTerrainGroup = OGRE_NEW Ogre::TerrainGroup(mSceneMgr, Terrain::ALIGN_X_Z, 513, worldSize.y);
			mTerrainGroup->setOrigin(Vector3(worldSize.y/2,0,worldSize.y/2)); // This is the CENTER
		}*/
		mCamera->setTerrainGroup(mTerrainGroup);
		...
		//mTerrainGlobals->set
		//mSceneMgr->setWorldGeometry(tTerrain); // Loading terrain here
		MovableObject::setDefaultQueryFlags(WORLDOBJECT_MASK);
//		MovableObject::addQueryFlags(AUTOCOLLISION_MASK);
		//mTerrainGroup = OGRE_NEW Ogre::TerrainGroup(mSceneMgr);
		mTerrainGroup->setFilenameConvention(worldName, Ogre::String("dat")); //  Ogre::String("BasicTutorial3Terrain")
		
		//mTerrainGroup->defineTerrain(5000, 5000);
		mTerrainGlobals->setMaxPixelError(mMaxPixelError);
		mTerrainGlobals->setCompositeMapDistance(mCompositeMapDistance);
/*		String resgroup = ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME;
		Image img;
		img.load(tTerrain, resgroup);
		mTerrainGroup->defineTerrain(0, 0, &img);*/
		//defaultimp.inputImage = &img;
		//mTerrainGroup->defineTerrain(0, 0, &defaultimp);
		Terrain::ImportData& defaultimp = mTerrainGroup->getDefaultImportSettings();
		defaultimp.terrainSize = mPageSize;
		defaultimp.inputScale = mMaxHeight; // height
		defaultimp.minBatchSize = mMinBatchSize;
		defaultimp.maxBatchSize = mMaxBatchSize;
		//defaultimp.pos = Vector3(2500,0,2500);
		// textures
		defaultimp.layerList.resize(3);
		defaultimp.layerList[0].worldSize = 100;
		defaultimp.layerList[0].textureNames.push_back("HyjalGrassA.PNG");
		defaultimp.layerList[0].textureNames.push_back("HyjalGrassA_s.PNG");
		defaultimp.layerList[1].worldSize = 100;
		defaultimp.layerList[1].textureNames.push_back("HyjalGrassBase.PNG");
		defaultimp.layerList[1].textureNames.push_back("HyjalGrassBase_s.PNG");
		defaultimp.layerList[2].worldSize = 100;
		defaultimp.layerList[2].textureNames.push_back("HyjalGrassB.PNG");
		defaultimp.layerList[2].textureNames.push_back("HyjalGrassB_s.PNG");
...
//				t->prepare(defaultimp);
		...
		if(!mDef->loadCritterSpawnList(worldName,critterSpawnLimit,critterSpawnList,critterRoamAreaList,tCritterSpawnFilename))
		{
			...
			critterSpawnList.clear();
			const Real tSize = (worldSize.x+worldSize.y)*(Ogre::Real)0.5;
			critterSpawnList.push_back(WorldCritter("Draw Point",(Ogre::Real)0.0001*tSize*(Ogre::Real)0.0001));
		...
		if(tTreesFilename.size()>0||tTree1!=""||tTree2!=""||tTree3!="")
		{
			...
			if(mDef->pagedGeometryOn)
			{
				...
				mTrees->addDetailLevel<BatchPage>(1500, (mDef->hasVertexProgram?(Ogre::Real)300:(Ogre::Real)0), Ogre::Any(), AUTOCOLLISION_MASK);		//Use batches up to 150 units away, and fade for 30 more units
				...
				mLargeTrees->addDetailLevel<BatchPage>(1500, (mDef->hasVertexProgram?(Ogre::Real)300:(Ogre::Real)0));
						...
						if(tLine.size()>1)
						{
							...
							for(int j=1;j<(int)tLine.size();j++)
							{
								...
								if(tSegment.size()==3)
								{
									...
									{
										if(!tTreesFilename[h].second)position.y = getTerrainHeight(position.x,position.z);
		...
		if(tBushesFilename.size()>0||tBush1!=""||tBush2!=""||tBush3!="")
		{
			...
			if(mDef->pagedGeometryOn)
			{
				...
				mBushes->addDetailLevel<BatchPage>(1000, (mDef->hasVertexProgram?(Ogre::Real)200:(Ogre::Real)0));
						...
						if(tLine.size()>1)
						{
							...
							for(int j=1;j<(int)tLine.size();j++)
							{
								...
								if(tSegment.size()==3)
								{
									...
									{
										if(!tBushesFilename[h].second)position.y = getTerrainHeight(position.x,position.z);
										Matrix3 m; m.FromEulerAnglesXYZ(Degree(pyr.x),Degree(pyr.y),Degree(pyr.z));
										Quaternion orientation;
										orientation.FromRotationMatrix(m);
		...
		for(int h=0;h<(int)tFloatingBushesFilename.size();h++)
		{
			...
			if(mDef->pagedGeometryOn)
			{
				...
				mFloatingBushes->addDetailLevel<BatchPage>(1000, (mDef->hasVertexProgram?(Ogre::Real)200:(Ogre::Real)0));
	...
	void unloadWorld()
	{
//		if(worldName != "Bootstrap" && worldName != "General")
//			ResourceGroupManager::getSingleton().unloadResourceGroup(worldName);
		mDynPaused = true;
		worldName = "";
		isInterior = false;
		mapEffect = "";
		...
		// Bullet
//		delete mDynWorld->getDebugDrawer();
//	    mDynWorld->setDebugDrawer(0);
		delete mDynWorld;
		mDynWorld = 0;
		mRigids.clear();
		mShapes.clear();
	...
	void addObject(const String &meshName, const Vector3 &position, const Vector3 &scale, const Real &pitch, const Real &yaw, const Real &roll, const String &playSound="", const String &matName="")
	{
		...
		// Bullet
		StaticMeshToShapeConverter *trimeshConverter = new StaticMeshToShapeConverter(ent);
		TriangleMeshCollisionShape *sceneTriMeshShape = trimeshConverter->createTrimesh();
		delete trimeshConverter;
		RigidBody *sceneRigid = new RigidBody(
			"WorldObjectRigid" + StringConverter::toString(numObjects),
			mDynWorld);
		...
		node->scale(scale);
		Matrix3 mx;
		mx.FromEulerAnglesYXZ(Radian(yaw), Radian(pitch), Radian(roll));
		Quaternion q(mx);
		sceneRigid->setStaticShape(node, sceneTriMeshShape, 0.1f, 0.8f, position, q);
		mShapes.push_back(sceneTriMeshShape);
		mRigids.push_back(sceneRigid);
	...
	void destroyAllObjects()
	{
		...
		std::deque<OgreBulletDynamics::RigidBody *>::iterator itBody = mRigids.begin();
		while (mRigids.end() != itBody)
		{   
			delete *itBody; 
			++itBody;
		}
	...
	void initializeStaticGeometry()
	{
		...
		for(int j=0;j<staticGeometryY;j++)
		{
			...
			sg->setOrigin(Vector3((Ogre::Real)i*1000,0,(Ogre::Real)j*1000));
