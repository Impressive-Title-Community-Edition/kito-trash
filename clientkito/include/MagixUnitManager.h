...
public:
			...
			else
			{
				...
				else if(unit->getPosition().y+tNode->getScale().x*15 < tWaterHeight)
				{
					...
					if(!mEffectsManager->hasBreatherOwner(tHeadEnt,"Bubbles"))
					{
						...
						mEffectsManager->createBreather(tHeadEnt,"Bubbles", Vector3(0,(Ogre::Real)1.2*tScale,(Ogre::Real)-0.4*tScale),tScale*(Ogre::Real)0.2);
					}
				}
				//Splash!
	...
	void updateMapEffects(MagixUnit *unit)
	{
		if(mWorld->getMapEffect()!="")
		{
			...
			if(!unit->allocFlags.mapEffect)
			{
				...
				mEffectsManager->createBreather(tHeadEnt,mWorld->getMapEffect(), Vector3(0,(Ogre::Real)1.2*tScale,(Ogre::Real)-0.4*tScale),tScale);
	...
	void clampUnitToTerrain(MagixAnimated *unit, bool clampToGround=false)
	{
		...
		Real tHeight = 0;
		tHeight = mWorld->getTerrainHeight(unit->getPosition().x,unit->getPosition().z);
		unit->setGroundHeight(tHeight);
		if(clampToGround)unit->setPosition(unit->getPosition().x,tHeight,unit->getPosition().z);
/*		TerrainGroup *mTerrainGroup = mWorld->getTerrainGroup();
		static Ray ray;
		ray.setOrigin(Vector3(unit->getPosition() + Vector3(0,50,0)));
...
//		unit->setGroundHeight(rayResult.position.y);
		//if(rayResult.hit)
		//	if(clampToGround)unit->setPosition(unit->getPosition().x,rayResult.position.y,unit->getPosition().z);
		//if(clampToGround)unit->setPosition(unit->getPosition().x,0,unit->getPosition().z);
	}
...
/*		TerrainGroup *mTerrainGroup = mWorld->getTerrainGroup();
		static Ray ray;
		ray.setOrigin(Vector3(Vector3(x,5000,z)));
		ray.setDirection(Vector3::NEGATIVE_UNIT_Y);
		TerrainGroup::RayResult rayResult = mTerrainGroup->rayIntersects(ray);
		tHeight = rayResult.position.y;*/
		/*static Ray updateRay;
		...
		//Real tHeight = 0;
		tHeight = mWorld->getTerrainHeight(x,z);
		//if(clampToGround)unit->setPosition(unit->getPosition().x,tHeight,unit->getPosition().z);
	...
	{
		if(target)target->createChatBubble(caption,mChatBubbleAttr,0,(target==mPlayer?(Ogre::Real)1:(Ogre::Real)0));
	}
	...
	void rewardCritter(MagixCritter *critter)
	{
		...
		if(tSkill.first!="")
		{
			unsigned char tStock = tSkill.second>1 ? (unsigned char)Math::RangeRandom((Ogre::Real)tSkill.second/(Ogre::Real)2,(Ogre::Real)tSkill.second) : 1;
