...
public:
	MagixHandler()
	{
		...
		mWorldEditor = new MagixWorldEditor();
...
//		delete mUnitManager;
	...
	void initialize(SceneManager *sceneMgr, RenderWindow *window)
	{
		...
		mSkyManager->initialize(sceneMgr, mDef, mCamera, mSoundManager, mTerrainGlobals);
		mItemManager->initialize(sceneMgr, mDef, mEffectsManager);
		mWorld->initialize(sceneMgr, window, mDef, mGameStateManager, mSoundManager, mCollisionManager, mSkyManager, mTerrainGlobals, mCamera);
		mWorldEditor->initialize(sceneMgr, window, mDef, mGameStateManager, mSoundManager, mCollisionManager, mSkyManager, mCamera, mWorld, mGui);
		...
		mGui->initialize(sceneMgr, window, mDef, mGameStateManager, mItemManager, mCritterManager, mUnitManager, mNetworkManager, mCamera, mSoundManager, mEffectsManager, mCollisionManager, mWorld);
		...
		mCamera->initialize(sceneMgr,getPlayer(),mCollisionManager,mGameStateManager);
		//mCamera->initWorld(mWorld);
	...
	bool update(const FrameEvent &evt)
	{
		...
		switch(mGameStateManager->getGameState())
		{
			...
			case GAMESTATE_STARTCHARSCREEN:
				...
				// Let's change the lighting for character creation
			...
			case GAMESTATE_QUIT:
				...
				contFlag = false;
			break;
			case GAMESTATE_STARTMAPEDITOR:
				mGui->showCharScreenOverlay(false);
				mGui->showBackdropOverlay(true,true);
				mGui->showMouse(false);
				mCamera->setCameraMode(CAMERA_FREE, false, true);
				if(mCamera->getCamera()->isAttached())mCamera->getActiveCamNode()->detachObject(mCamera->getCamera());
				mCamera->getCamera()->setPosition(3200,120,2770);
				mCamera->getCamera()->setOrientation(Quaternion(Degree(-40),Vector3::UNIT_Y));
				mSkyManager->showSky(true);
				...
				mSkyManager->setWeatherCycle("");
				mSkyManager->update(evt);
				mWorldEditor->loadWorld("Default");
				mWorldEditor->update(evt);
				mGui->showBackdropOverlay(false);
				mGui->showWorldEditorOverlay(true);
				mGui->showWorldEditorPanel(1);
				mGui->showMouse(true);
				// Let's change the lighting for character creation
				mSceneMgr->setAmbientLight(ColourValue(1, 1, 1));
				mGameStateManager->setGameState(GAMESTATE_INMAPEDITOR);
			break;
			case GAMESTATE_INMAPEDITOR:
				updateWorldEditorScreen(evt);
			break;
		}

		return contFlag;
	}
	void updateWorldEditorScreen(const FrameEvent &evt)
	{
		updateCommand();
		mCamera->update(evt);
		mGui->update(evt);
	...
	void updateCommand()
	{
		...
		{
			String tMap = "Default";
			Vector2 tPoint = Vector2(2500,2500);
			unsigned char tDimension = 0;
		...
		{
			mDef->saveHomePoint("Default",Vector2(2500,2500),0);
	...
	MagixWorldEditor* getWorldEditor()
	{
		return mWorldEditor;
	}
};
