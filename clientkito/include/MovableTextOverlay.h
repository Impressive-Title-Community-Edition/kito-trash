...
public:
	...
	int getPixelsTop() {return Ogre::OverlayManager::getSingleton().getViewportHeight() * (int)mpOvContainer->getTop();}
	int getPixelsBottom() {return Ogre::OverlayManager::getSingleton().getViewportHeight() * ((int)mpOvContainer->getTop() + (int)mpOvContainer->getHeight());}
	int getPixelsLeft() {return Ogre::OverlayManager::getSingleton().getViewportWidth() * (int)mpOvContainer->getLeft();}
	int getPixelsRight() {return Ogre::OverlayManager::getSingleton().getViewportWidth() * ((int)mpOvContainer->getLeft() + (int)mpOvContainer->getWidth());}
...
void MovableTextOverlay::update(const Real &timeSincelastFrame, bool placeAtTop, const Real &borderSizeOffset, bool chaseUpdate)
{
	...
	if(chaseUpdate)
	{
		tX = (tX - mpOvContainer->getLeft())*(Ogre::Real)0.08;
		tY = (tY - mpOvContainer->getTop())*(Ogre::Real)0.08;
...
, mFontName("")
, mMaterialName("")
, mCharHeight((Ogre::Real)charHeight)
