...
public:
	...
	void updateFaceDirection(const FrameEvent &evt)
	{
		...
		if(Math::Abs(tYaw)>Degree(180))tYaw -= Degree(360) * (tYaw<Degree(0)?(Ogre::Degree)-1:(Ogre::Degree)1);
	...
	void updatePhysics(const FrameEvent &evt)
	{
		...
		//Terminal Velocity
		if(Math::Abs(mForce.y)>TERMINAL_VEL*evt.timeSinceLastFrame*(antiGravity?0.5:1))mForce.y = (Ogre::Real)TERMINAL_VEL * (mForce.y<0?(Ogre::Real)-1:(Ogre::Real)1)*evt.timeSinceLastFrame*(antiGravity?(Ogre::Real)0.5:(Ogre::Real)1);
	...
	void yaw(const Degree &angle, bool instant=false)
	{
		...
		if(instant)
		{
			...
			if(tCurrentYaw > Degree(360))tCurrentYaw -= Degree(360);
			Degree tYaw = faceDirectionYaw - tCurrentYaw;
			if(Math::Abs(tYaw)>Degree(180))tYaw -= Degree(360) * (tYaw<Degree(0)?(Ogre::Real)-1:(Ogre::Real)1);
